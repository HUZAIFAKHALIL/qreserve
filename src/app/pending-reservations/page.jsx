...
export default function PendingReservations() {
  ...
  const handleToggleReservationSelection = (reservation) => {
    setSelectedReservations(prev => 
      prev.some(r => r.serviceId === reservation.serviceId)
        ? prev.filter(r => r.serviceId !== reservation.serviceId)
        : [...prev, reservation]
    );
  };
  ...
  const handleConfirmEditReservation = (event) => {
    ...
    if (!startDate || (!endDate && ['hotel', 'car', 'hall', 'restaurant'].includes(editingReservation.type))) {
      setError("Please provide both start and end dates.");
      return;
    }

    if (endDate && ['hotel', 'car', 'hall', 'restaurant'].includes(editingReservation.type) && new Date(startDate) >= new Date(endDate)) {
      setError("End date should be later than start date.");
      return;
    }
    ...
    const filteredUpdatedReservations = updatedReservations.map((reservation) => {
      // Ensure a valid price is always set
      const price = reservation.specificService 
        ? reservation.specificService.price * (reservation.quantity || 1)
      ...
      const baseReservation = {
        ...
        startDate: reservation.startDate,
        endDate: reservation.endDate,
        price: price,  // Consistently use 'price'
        quantity: reservation.quantity || 1
      };
  ...
  const handleDelete = (reservation) => {
    if (
      ...
      // Remove from selected reservations if it was selected
      setSelectedReservations(prev => 
        prev.filter(r => r.serviceId !== reservation.serviceId)
      );
    }
  ...
  const handleConfirmSelectedReservations = async () => {
    ...
    // Store only selected reservations in localStorage
    const filteredReservations = selectedReservations.map((reservation) => {
      const baseReservation = {
        userId: reservation.userId,
        userEmail: reservation.userEmail,
        serviceId: reservation.serviceId,
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {reservations.map((reservation) => (
          <div
            key={reservation.serviceId}
            className={`bg-white rounded-xl shadow-lg overflow-hidden transform transition-all duration-300 hover:shadow-2xl hover:scale-[1.02] cursor-pointer ${
              selectedReservations.some(r => r.serviceId === reservation.serviceId)
              ...
              {reservation.specificService && (
                <div className="flex items-center text-gray-600 mb-3">
                  <span className="font-medium">
                    ...
                    (reservation.type === 'hotel' ? `${reservation.specificService.roomType} Room` :
                     reservation.type === 'car' ? reservation.specificService.carModel : 
                     reservation.type === 'restaurant' ? `${reservation.specificService.tableType || 'Standard'} Table` :
                     reservation.specificService.serviceType || 'Standard')}
                  </span>
                </div>
              )}
      ...
      {reservations.length > 0 && (
        <div className="mt-12 text-center">
          <button
            ...
            className={`text-white py-3 px-8 rounded-lg transition-colors duration-200 flex items-center justify-center mx-auto ${
